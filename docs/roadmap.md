...
フェーズ番号は概念的な順序である。ただし実装は、必ずしも番号順に固定しない。
...
Phase 1 を固める
  ↓
Phase 4 で JouJou に最小統合
  ↓
Phase 2 Decision Logger
  ↓
Phase 3 Policy Gate
  ↓
...
理由は、Koguchi 単体で抽象化しすぎるより、JouJou の `create_todo` という実用ケースに当てることで、`ActionEnvelope`、`UNCONFIRMED`、reconciliation、redaction、Decision Logger の不足が早く見えるからである。

ただし、Phase 1 の不変条件が固まる前に Provider 統合を広げてはいけない。副作用経路が分裂すると、後から塞ぐコストが高くなる。
...
直近では、Phase 1 の意味論固定と監査基盤強化を優先する。

現在の優先 Issue は次である。

```text
#7  親ディレクトリ作成副作用の扱いを確定する
#8  Envelopeなしの扱いを REJECTED と例外のどちらに統一するか決める
#9  ExecutionStore Protocol に committed() を追加する
#10 hash chain 検証 API verify_chain() を追加する
```
...
#7 と #8 は、Koguchi の副作用意味論を固定する。#9 と #10 は、Store と監査可能性を強化する。

Phase 1 の意味論が固まったら、JouJou の `KoguchiAuditGate` に進む。
...
次回は、Phase 1 の完了条件をより具体的なチェックリストに落とす。

その後、Phase 4 の JouJou 統合に向けて、`KoguchiAuditGate` の最小 API、`TodoService` との接続、`UNCONFIRMED` の Result / Error 型を定義する。
